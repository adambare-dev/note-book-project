...
function Note_book() {
  ...
  return (
    <div className="task-tracker-container">
      ...
      <div className="items-group">
        <ul>
          ...
          {filtredNotes.map((item) => {
            return (
              <li key={item.id} className={item.complited ? "complited" : ""}>
                <input
                  type="checkbox"
                ...
                >
                  <FaTrashAlt title="delete this item" />
                </button>
              </li>
            );
          })}
      ...
      <p className="note">
        {items.length < 1 ? "No Notes YET" : items.length + " Notes remaining"}
      </p>
    </div>
  );
}
